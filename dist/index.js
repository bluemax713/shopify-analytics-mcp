...
if (stores.size === 0) {
    console.error("[shopify-analytics-mcp] No stores configured. Set SHOPIFY_STORES=ra,pb and " +
...
server.tool("list_stores", "List the Shopify stores available for analytics queries", {}, async () => {
    const list = Array.from(stores.values()).map((s) => ({ id: s.id, domain: s.domain }));
    return {
        content: [{ type: "text", text: JSON.stringify(list, null, 2) }],
    };
});
server.tool("run_shopifyql", "Run a ShopifyQL query against a Shopify store. Returns tabular results. " +
    "Example queries: " +
...
server.tool("query_orders_by_date", "Query orders and compute sales metrics for a date range. Works on all Shopify plans. " +
